...
async def on_message(message):
    ...
    if lvl > xp_data[uid]["level"]:
        ...
        await message.channel.send(f"🎉 {message.author.mention} passe au niveau **{lvl}** !", delete_after=10)
...
async def kick(ctx, member: discord.Member, *, reason="Aucune raison"):
    ...
    em = discord.Embed(title="👢 Kick", color=0xE67E22, description=f"{member} expulsé par {ctx.author}\nRaison: {reason}")
...
async def ban(ctx, member: discord.Member, *, reason="Aucune raison"):
    ...
    em = discord.Embed(title="🔨 Ban", color=0xE74C3C, description=f"{member} banni par {ctx.author}\nRaison: {reason}")
    await ctx.send(embed=em); await log_action(ctx.guild, em)
...
async def mute(ctx, member: discord.Member, minutes: int=10, *, reason="Aucune raison"):
    ...
    em = discord.Embed(title="🔇 Mute", color=0xF1C40F, description=f"{member} muté {minutes}min par {ctx.author}\nRaison: {reason}")
    await ctx.send(embed=em); await log_action(ctx.guild, em)
...
async def help_bot(ctx):
    ...
    em.add_field(name="Stats", value="`!stats` `!serverstats`", inline=False)
    em.add_field(name="Communauté", value="`!anniversaire` `!meteo`", inline=False)
...
async def blague(ctx):
    blagues = [
        ...
        "Un escargot se fait écraser par une tortue. La police lui demande ce qui s'est passé. Il répond : je sais pas, ça allait trop vite !",
...
async def anti_spam_insultes(message):
    ...
    if len(message_counts[uid]) >= 5:
        ...
        await message.channel.send(f"🔇 {message.author.mention} muté 2 minutes pour spam !", delete_after=10)
        message_counts[uid] = []
...
async def meteo(ctx, *, ville: str):
    try:
        ...
        em = discord.Embed(title=f"🌡️ Météo à {ville.title()}", color=0x3498DB)
        ...
        await ctx.send(f"❌ Impossible de trouver la météo pour **{ville}**.")
...
async def stats(ctx, membre: discord.Member = None):
    ...
    jours = (maintenant - membre.joined_at).days
    jours_discord = (maintenant - membre.created_at).days
    sorted_users = sorted(xp_data.items(), key=lambda x: x[1]["xp"], reverse=True)
    ...
    em = discord.Embed(title=f"📊 Profil de {membre.display_name}", color=membre.color if membre.color.value != 0 else 0x5865F2)
    ...
    em.add_field(name="📅 Sur le serveur", value=f"**{jours} jours** ({membre.joined_at.strftime('%d/%m/%Y')})", inline=True)
    em.add_field(name="🎂 Compte créé", value=f"Il y a **{jours_discord} jours**", inline=True)
...
async def serverstats(ctx):
    g = ctx.guild
    maintenant = datetime.datetime.now(datetime.timezone.utc)
    age = (maintenant - g.created_at).days
    ...
    em.add_field(name="📅 Créé il y a", value=f"**{age} jours**", inline=True)
    await ctx.send(embed=em)
